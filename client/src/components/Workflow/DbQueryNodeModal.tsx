...
interface DbQueryNodeModalProps {
  node: any;
  isOpen: boolean;
  onRequestClose: () => void;
...
const DbQueryNodeModal: React.FC<DbQueryNodeModalProps> = ({ node, isOpen, onRequestClose, onSave }) => {
  ...
  return (
    <Dialog open={isOpen} onClose={onRequestClose} maxWidth="sm" fullWidth>
      <DialogTitle>DB Query Node Configuration</DialogTitle>
      <DialogContent dividers>
        <Stack spacing={2} component="form" onSubmit={handleSubmit}>
          <TextField select label="Data Source" name="dataSource" value={data.dataSource || ''} onChange={handleChange} fullWidth>
            <MenuItem value="Primary DB">Primary DB</MenuItem>
            <MenuItem value="Secondary DB">Secondary DB</MenuItem>
          </TextField>
...
export default DbQueryNodeModal;
